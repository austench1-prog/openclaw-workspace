...
# Date: 2026-06-22 | Status: DISCUSSION DRAFT (Dragon 起草, 待 President 审)
# Scope: 定义 OC ↔ 小白研究中心 ↔ Hermes ↔ President 之间的结构化文件交换格式。
#        架构 + 格式 ONLY。不实施。落地需 President 批准。
...
3. **各守车道。** OC 写执行/任务，小白写研究输出，Hermes 写运营，总裁 写批准。
4. **只读快照 + President 唯一批准关口。** 小白只读 OC 快照、只写自己 Output；批准后才成正式 release。
5. **schema 锁死。** 坏单 = 异常处理，不静默吞。
6. **YAML 头 + Markdown 正文** 统一格式（机器读头、人读正文）。

---

## 1. 总线物理形态（🔒 President 2026-06-22 锁定：一条总线）
...
- 权限模型（GitHub 实现）：
  ...
  | President | 全部 | `03_Approvals/{Approved,Rejected}` |
- (备选已否决：两条总线方案不采用。)
...
> 现阶段（人工在前操作）= **3份**。未来若半自动化（OC自动发任务/小白定时跑/Hermes自动归档/总裁只看待审清单），再把"研究"与"质量门"拆开成4份。现在不拆,避免结构过重。
> **关键认知：小白只是电脑/分析工具,不是决策主体。** Claude 的 PASS/REVISE/REJECT = 研究建议,不是审批。最终权力在 总裁。
...
scope: "仅采用参数风险结论,不采用新加仓逻辑"   # President 限定采用范围
---
## President 批注
## 采用/不采用 明细
```
...
OC/小白 把 ② 提请 → 03_Approvals/Pending/
  ↓ President 审(②里的 PASS/REVISE/REJECT 只是建议,President 才是关口)
President 写 ③ → Approved/ 或 Rejected/
...
## 6b. 第一轮 skill 白名单（🔒 President 2026-06-22 锁定）
...
---
*DRAFT v1.0 | 2026-06-22 | Dragon 起草 | 不实施待 President 审*
