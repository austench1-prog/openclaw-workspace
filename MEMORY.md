...
> Chairman 陪了一整天就为让我搞懂这套原理。**重点不是4个XML文件，是"理解模板的思维框架"——这个最容易丢。读到这里=立刻想起来，不要再从头绕一遍。**
...
- 远程能力：我SSH能读写推送模板XML(全自动)；但**下载回放数据/切Playback/图形挂单=GUI**，我做不了(NinjaTrader无命令行接口),需Chairman在AnyDesk操作。OIF(incoming文件夹)能程序化下单但不能下数据。
...
### 现状（2026-06-22）
ATM策略落地**暂停**(Chairman定)——ATM边界已清楚:它当"出场管家"够用,但装不下完整加仓策略。要全自动1+3加仓需上NinjaScript(B路线,现阶段不做)。4模板已上温总PC+workspace,干净无残留。
...
## 交易时段纪律（⏸️ 暂未生效 — Chairman 2026-06-22 暂停）

**⏸️ 当前状态（2026-06-22 锁定）：交易时段纪律暂不生效。** 真交易尚未开始；本纪律需等 Chairman **正式确认"开始真交易"后**才启动。**现在 = 系统建设期**，不受盯盘时段限制，可正常进行较重任务（进尼家核 ATM、批量整理、大 subagent 等）随时可做。
**重启条件：** Chairman 明确说"开始（真交易）"→ 立即恢复下面全部纪律。
