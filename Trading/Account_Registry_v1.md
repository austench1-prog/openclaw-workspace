...
✅ **风控入口（平台）— 6/16 锁定，不是新问题：** CQG=底层地基，Tradovate=上层前端，数据同源。**风控要分两扇门设：MFF → Tradovate Risk Settings；TPT → CQG / TPT 后台调 CQG。** 不是“TPT 不能设风控”，是“在另一扇门设”。全文见 `System_Landing_Framework_v1.md` L88-99。强制平仓原理两者统一（底层都是 CQG 订单管理）。
