...
| 平台 | 账号 | 计划 | 阶段 | 盈利目标 | Max DD | DD类型 | 一致性 | 最低天数 | $1500/日自动平仓 | 规则来源 | 状态 |
|---|---|---|---|---|---|---|---|---|---|---|---|
...
## ⭐ MFF Rapid 通过路线图 (3 天最理想方案)

**一致性数学(锁定,与 TPT 算法一致):**
- 规则:单日盈利 ≤ 总盈利 × 50% → 反推 **总盈利 ≥ 最大单日 × 2**,且 ≥ $3,000。
- **2 天通过 ≈ 不可能:** 两天必须分毫不差各 $1,500(任一天 >$1,500 即破 50%)。实操不可行。

**✅ 3 天通过方案(最稳):**

| 交易日 | 目标盈利 | 说明 |
|---|---|---|
| 第 1 天 | **~$1,500**(不超) | 已完成 2026-06-16 (045: +$1,481 / 046: +$1,476) |
| 第 2 天 | **~$1,500**(不超) | 待执行 |
| 第 3 天 | **小额(数刀~几十刀)** | 凑够第 3 个交易日 + 总额越过 $3,000 |

- 总额 ≈ $3,001,最大单日占比 ≈ 49.98% < 50% ✅,且 ≥ 2 天 ✅ → **通过**。

**🚨 唯一红线:第 1、2 天任何一天都不能 > $1,500**(否则那天 > 总额 50%,破一致性)。
- 今早 045/046 停在 $1,481/$1,476 → 完美卡线下,留安全垫。✅

**风控触发逻辑(待落地到平台/系统):**
```
if mff_daily_pnl >= 1500 * 0.90:   # 接近 $1,500
    发警告("MFF 今日接近 $1500 一致性上限")
if mff_daily_pnl >= 1500:
    FLATTEN_ALL()  # 平台/系统自动平仓(最高目标 = Tradovate 每日最高盈利风控自动平仓)
```
...
- ⬜ **$1500 自动平仓 落地** — 通过 Tradovate「每日最高盈利」风控功能,交易中自动平仓(白名单 #1,最高目标)。
