...
//
// STUDY: When price retraces to Level75 (Low + 0.75 x Range) of a bullish HTF candle,
//        does it hit ImpulseHigh (UP) or Level25 (DOWN) first?
...
{
    public class Level75StudyV1 : Strategy
    {
        // ---- CSV output -------------------------------------------------------
        ...
        protected override void OnStateChange()
        {
            ...
            {
                Description  = "NQ Level75 Study v1 - trigger=L75 UP=High DOWN=L25 - no orders";
                Name         = "Level75StudyV1";
            ...
            else if (State == State.DataLoaded)
            {
                ...
                _csv5mPath  = Path.Combine(dir, "Level75Study_5m_"  + ts + ".csv");
                _csv15mPath = Path.Combine(dir, "Level75Study_15m_" + ts + ".csv");
        ...
        private void Monitor(ref Evt ev, string tf, double bH, double bL, DateTime bT)
        {
            ...
            if (!ev.Touched)
            {
                // Trigger: Level75 touch (price pulls back to 75% of candle range)
                if (bL <= (ev.L + 0.75 * ev.Range))
                {
                    ev.Touched   = true;
                    ev.TouchTime = bT;
                    ev.MaxAfter  = bH;
        ...
        private void WriteSummary()
        {
            ...
            {
                sw.WriteLine("=================================================================");
                sw.WriteLine("  NQ LEVEL75 STUDY v1 -- trigger=L75 UP=High DOWN=L25");
