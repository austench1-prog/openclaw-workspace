# APM LLC — 公司主档案
> 最后更新：2026-04-04

---

## 基本资料

| 项目 | 内容 |
|---|---|
| 公司全名 | APM LLC |
| 成立州 | Nevada |
| 成立日期 | January 8, 2026 |
| EIN | 35-2947076（存加密磁盘）|
| 管理结构 | Member Managed |
| 成员 | Austin Chien — Sole Member — 100% |
| 默认税务身份 | Disregarded Entity（Single-Member LLC）|
| 已选择税务身份 | S-Corporation（S-Corp election 已做）|

---

## 业务定位（银行 KYC 口径）

> 来源：Business Activity Confirmation Letter（2026-03-24，致 Citibank）

- **业务性质：** 管理与运营实体，专注金融市场运营管理
- **收入来源：** 来自第三方 Prop Firm 的绩效分成（基于管理监督）
- **业务范围：** 纯运营/行政职能，不提供投资建议，不持有或管理客户资金
- **税务结构：** S-Corp，管理专业支出、行政成本及员工薪酬

---

## Payroll 设置（S-Corp 要求，三步）

| 步骤 | 内容 | 状态 |
|---|---|---|
| 1 | 注册 IRS 联邦 Payroll（Form 941 / Form 940）| 待完成 |
| 2 | 注册 Nevada DETR（州失业保险）https://uitax.nvdetr.org | 待完成 |
| 3 | 选择 Payroll 平台（推荐 Gusto）| 待完成 |

> ⚠️ S-Corp 必须给 owner 发合理工资（Reasonable Salary），剩余利润可以以 Distribution 取出，可降低自雇税

---

## 账户状态

| 账户 | 机构 | 状态 |
|---|---|---|
| 银行账户 | Citibank（开户进行中）| 进行中 |
| 券商账户 | 待填写 | 待填写 |

---

## 合规事项

| 项目 | 截止日期 | 状态 |
|---|---|---|
| Nevada Annual Report | 待填写 | 待填写 |
| BOI Filing | 待填写 | 待填写 |
| IRS Form 941（每季度）| 每季度末 | 待启动 |
| IRS Form 940（每年）| 每年1月31日 | 待启动 |

---

## 文件索引（原件存 iCloud Drive）

| 文件 | 云盘路径 | 状态 |
|---|---|---|
| APM_LLC_Corporate_Kit.pdf | `Business/APM_LLC/01_Formation/` | ⬜ 待存入 |
| APM_LLC_Professional_Corporate_Book.pdf | `Business/APM_LLC/01_Formation/` | ⬜ 待存入 |
| APM_LLC_Operating_Agreement_clean.pdf | `Business/APM_LLC/01_Formation/` | ⬜ 待存入 |
| APM_LLC_Business_Activity_Confirmation.pdf | `Business/APM_LLC/07_Contracts_Letters/` | ⬜ 待存入 |
| APM_SCorp_Payroll_Setup_OnePage_EN.pdf | `Business/APM_LLC/06_Payroll/` | ⬜ 待存入 |
| EIN Letter (CP575) | `Business/APM_LLC/02_IRS_Tax/` | ⬜ 待存入 |

---

## 当前待办

- [ ] 把以上5个 PDF 存入 iCloud Drive 对应路径
- [ ] 完成 Banking Resolution 签字（日期空白待填）
- [ ] 注册 IRS 联邦 Payroll
- [ ] 注册 Nevada DETR
- [ ] 选择 Payroll 平台（建议 Gusto）
- [ ] 确认 Nevada 年报截止日
- [ ] Citibank 开户进度跟进
